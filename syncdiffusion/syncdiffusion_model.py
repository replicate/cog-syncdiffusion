...
class SyncDiffusion(nn.Module):
    def __init__(self, device="cuda", sd_version="2.0", hf_key=None):
        ...
        if hf_key is not None:
            ...
            model_key = "stable-diffusion-2-base"  # in the original repo it was called "stabilityai/stable-diffusion-2-base"
        ...
        # it is modified in order to use local model file (it is not pretrained, using the default vgg model), but not working
        # see more, https://github.com/richzhang/PerceptualSimilarity/blob/master/lpips/lpips.py
        # weights are downloaded from https://download.pytorch.org/models/vgg16-397923af.pth
        # self.percept_loss = lpips.LPIPS(
        #     net="vgg", pretrained=True, model_path="vgg16-397923af.pth"
        # ).to(self.device)
        self.percept_loss = lpips.LPIPS(net="vgg").to(self.device)  # original code
